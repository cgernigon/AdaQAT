...
def main():
    ...
    for epoch_ in range(int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if len(student_model.CEA) != 0:
                # Exploration phase
                outputs, outputsw, outputsa = student_model(input_ids,segment_ids,input_mask)
            else:
                # Switch to standard QAT when the exploration phase is completed
                outputs = student_model(input_ids,segment_ids,input_mask)

            if len(student_model.CEA) != 0:

                logits, atts, reps = outputs
                ...
                student_logits, student_atts, student_reps = outputs
                with torch.no_grad():
            ...
            if len(student_model.CEA) != 0:
                logitsw, _, _ = outputsw
                start_logitsw, end_logitsw = logitsw
                ignored_index = start_logitsw.size(1)
                loss_fctw = torch.nn.CrossEntropyLoss(ignore_index=ignored_index)
                start_lossw = loss_fctw(start_logitsw, start_positions)
                end_lossw = loss_fctw(end_logitsw, end_positions)
                lossw = (start_lossw + end_lossw) / 2

                logitsa, _, _ = outputsa
                start_logitsa, end_logitsa = logitsa
                ignored_index = start_logitsa.size(1)
                loss_fcta = torch.nn.CrossEntropyLoss(ignore_index=ignored_index)
                start_lossa = loss_fcta(start_logitsa, start_positions)
                end_lossa = loss_fcta(end_logitsa, end_positions)
                lossa = (start_lossa + end_lossa) / 2

                derivwidthw = (loss.item() - lossw.item()) + lmbda*student_model.gradBitWidth()[1] # bit-width gradient calculation for the weights
                derivwidtha = (loss.item() - lossa.item()) + lmbda*student_model.gradBitWidth()[0] # bit-width gradient calculation for the activations

                # Calculating and updating the bit-width value (both weight and activation)
                student_model.update(derivwidthw, derivwidtha)

            if len(student_model.CE) == 0:
                # Modifying lambda for learning activation bit-widths per layer 
                # Hyper-parameter to modify according to the model used (/!\ a little challenging to find the right value)
                lmbda = 0.01

            if step and step % 1 == 0:
                # Revaluing the layer to update in mixed precision
                student_model.CEdiv()
            ...
            if global_step % args.eval_step == 0 or global_step == num_train_optimization_steps-1:
                ...
                print(student_model.listOfWidth())
                print(student_model.list_bitops)
                print('BitOps: {:.3f}, Model weight memory size: {:.2f}, Weight compression rate: {:.2f}, Weight bit-width mean: {:.2f}, Activation bit-width mean: {:.2f}'.format(student_model.model_bitops, student_model.model_w_size, student_model.W_compresion, student_model.W_mean, student_model.A_mean))
                print('model weight bit-widths converged :', student_model.Wconvergence, ', model activation bit-widths converged :',student_model.Aconvergence, ', layer weight-bit widths converged :',len(student_model.CE) == 0, ', layer activation bit-widths converged :',len(student_model.CEA) == 0)
